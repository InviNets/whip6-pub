...
static const void* lastAuxRamImage = NULL;
static volatile bool scifReady = false;


/// Import OSAL
#define SCIF_INCLUDE_OSAL_C_FILE
#include "scif_osal_whip6.c"
...
SCIF_RESULT_T scifInit(const SCIF_DATA_T* pScifDriverSetup) {
    ...
    AUXWUCClockEnable(AUX_WUC_SMPH_CLOCK | AUX_WUC_AIODIO0_CLOCK |
            AUX_WUC_AIODIO1_CLOCK | AUX_WUC_TIMER_CLOCK | AUX_WUC_ANAIF_CLOCK |
            AUX_WUC_TDCIF_CLOCK | AUX_WUC_ADI_CLOCK | AUX_WUC_OSCCTRL_CLOCK);
    ...
    if (lastAuxRamImage != scifData.pAuxRamImage) {
        memcpy((void*) AUX_RAM_BASE, scifData.pAuxRamImage, scifData.auxRamImageSize);
        lastAuxRamImage = scifData.pAuxRamImage;
    }
...
void scifUninit(void) {
    ...
    // Disable clocks
    AUXWUCClockDisable(AUX_WUC_SMPH_CLOCK | AUX_WUC_AIODIO0_CLOCK |
            AUX_WUC_AIODIO1_CLOCK | AUX_WUC_TIMER_CLOCK | AUX_WUC_ANAIF_CLOCK |
            AUX_WUC_TDCIF_CLOCK | AUX_WUC_ADI_CLOCK);
} // scifUninit
...
void scifClearReadyIntSource(void) {
